namespace Tests
{
	...
	public sealed class MultiEndianBinaryReaderTests : TestClass
	{
		...
		public void TestBinaryReadMethods()
		{
			...
			Assert.Equal(int16l, 0x0201);
			Assert.Equal(uint16l, 0x0201);
			Assert.Equal<int>(int32l, 0x04030201);
			Assert.Equal<uint>(uint32l, 0x04030201);
			Assert.Equal<long>(int64l, 0x0807060504030201);
			Assert.Equal<ulong>(uint64l, 0x0807060504030201);

			Assert.Equal(int16b, 0x0102);
			Assert.Equal(uint16b, 0x0102);
			Assert.Equal<int>(int32b, 0x01020304);
			Assert.Equal<uint>(uint32b, 0x01020304);
			Assert.Equal<long>(int64b, 0x0102030405060708);
			Assert.Equal<ulong>(uint64b, 0x0102030405060708);

			// We don't have direct casts for these (they convert, not memory cast), so let's just compare for inequality.
			Assert.NotEqual(singlel, singleb);
			Assert.NotEqual(doublel, doubleb);
		}
