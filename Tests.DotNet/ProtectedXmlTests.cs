...
namespace Tests
{
	public sealed class ProtectedXmlTests : TestClass
	{
		...
		public void EncryptAndSign_AppearsToEncryptAndSign()
		{
			...
			// It looks encrypted.
			Assert.Equal("EncryptedData", document.DocumentElement.LocalName);

			// And it looks signed.
			Assert.Equal(1, document.DocumentElement.ChildNodes.Cast<XmlNode>().Count(n => n.LocalName == "Signature"));
		}
		...
		public void VerifyAndDecrypt_WithNoDecryptionKey_Fails()
		{
			...
			Assert.Throws<CryptographicException>(() => Helpers.ProtectedXml.VerifyAndDecrypt(document, out signedBy, _cPrivate));
		}
		...
		public void EncryptAndSign_WithWeakEncryptionKey_Fails()
		{
			...
			Assert.Throws<ArgumentException>(() => Helpers.ProtectedXml.EncryptAndSign(document, _weak1Public, _aPrivate));

			document = LoadDocument();

			Assert.Throws<ArgumentException>(() => Helpers.ProtectedXml.EncryptAndSign(document, _weak2Public, _aPrivate));
		}
		...
		public void EncryptAndSign_WithWeakSigningKey_Fails()
		{
			...
			Assert.Throws<ArgumentException>(() => Helpers.ProtectedXml.EncryptAndSign(document, _aPublic, _weak1Private));

			document = LoadDocument();

			Assert.Throws<ArgumentException>(() => Helpers.ProtectedXml.EncryptAndSign(document, _aPublic, _weak2Private));
		}
		...
		public void VerifyAndDecrypt_WithWeakKeyJustTaggingAlong_Fails()
		{
			...
			Assert.Throws<ArgumentException>(() => Helpers.ProtectedXml.VerifyAndDecrypt(document, out signedBy, _aPrivate, _weak1Private));

			Assert.Throws<ArgumentException>(() => Helpers.ProtectedXml.VerifyAndDecrypt(document, out signedBy, _aPrivate, _weak2Private));
		}
